...
$attendance_extraction_mode = trim($_POST['attendance_extraction_mode'] ?? 'both');
...
// attendance_extraction_mode 유효성 검사
if (!in_array($attendance_extraction_mode, ['both', 'title', 'body'])) {
    $attendance_extraction_mode = 'both';
}
$attendance_extraction_mode = mysqli_real_escape_string($conn, $attendance_extraction_mode);
...
if ($site_id > 0) {
    ...
    $query = "UPDATE monitor_sites SET 
                ...
                attendance_extraction_mode = '$attendance_extraction_mode',
    ...
    $query = "INSERT INTO monitor_sites 
                (site_name, site_url, site_type, target_selector, attendance_extraction_mode, check_interval, is_active, follow_redirect, enable_notification, created_at, updated_at)
              VALUES 
                ('$site_name', '$site_url', '$site_type', '$target_selector', '$attendance_extraction_mode', $check_interval, $is_active, $follow_redirect, $enable_notification, NOW(), NOW())";
}
